# section-pagination Specification

## Purpose
How the UI bounds each digest section for display: the UI owns the display bound rather than the
contract, readers page bidirectionally through the complete candidate set, every item keeps its
feedback affordance at any depth, paging resets when the document changes, and the pager controls
are exposed accessibly.
